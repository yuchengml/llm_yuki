...
> record of *why* each decision (D1–D24) was made — that folder is not modified by these docs and should stay
...
| [`cli-and-cost-ledger.md`](./cli-and-cost-ledger.md) | The `llm-yuki compile` CLI, LLM client config, `cost_ledger.jsonl`, D24's per-run `stat_<timestamp>.md` report |
