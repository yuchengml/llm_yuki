...
| `description` | Short **one-sentence** summary for `concepts/index.md` entries (D23 §5.4, extended beyond the original decision — see `TODO.md`) — distinct from `summary`, which may be a full multi-section write-up | LLM; `DefaultMerger._merge_concept_pair` (`new or old` on merge — **not** D22's 3-layer `summary` protection, since this is index metadata, not body content) | `MarkdownWriter._concept_index_entries` — falls back to `f"{concept_title}: {summary}"` when empty |
