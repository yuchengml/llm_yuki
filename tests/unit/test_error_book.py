...
def test_update_error_book_creates_open_entry_with_root_cause_and_constraint() -> None:
    ...
    entries = book.update_error_book([_issue()], batch_id=1, writer=_FakeWriter())
...
def test_update_error_book_writes_one_log_line_per_issue() -> None:
    """Proposal §4.4: every UpdateErrorBook call writes a matching log.md audit line."""
    writer = _FakeWriter()
    book = ErrorBook()

    entries = book.update_error_book([_issue()], batch_id=1, writer=writer)

    assert len(writer.log_events) == 1
    assert str(1) in writer.log_events[0]
    assert entries[0].id in writer.log_events[0]
    assert entries[0].error_type in writer.log_events[0]


def test_update_error_book_merges_repeat_occurrence_into_existing_entry() -> None:
    writer = _FakeWriter()
    book = ErrorBook()
    book.update_error_book([_issue(refs=["a"])], batch_id=1, writer=writer)

    book.update_error_book([_issue(refs=["b"])], batch_id=2, writer=writer)
    ...
    assert len(writer.log_events) == 2  # one for the initial open, one for the recurrence


def test_active_constraints_returns_only_open_entries() -> None:
    book = ErrorBook()
    book.update_error_book([_issue()], batch_id=1, writer=_FakeWriter())
...
def test_verify_and_close_closes_dangling_link_once_target_exists() -> None:
    book = ErrorBook()
    writer = _FakeWriter()
    book.update_error_book([_issue(refs=["water"])], batch_id=1, writer=writer)
    ...
    assert len(writer.log_events) == 2  # one from UpdateErrorBook, one from VerifyAndClose
    assert "VerifyAndClose" in writer.log_events[-1]
    assert closed[0].id in writer.log_events[-1]


def test_verify_and_close_leaves_entry_open_when_still_unresolved() -> None:
    book = ErrorBook()
    book.update_error_book([_issue(refs=["water"])], batch_id=1, writer=_FakeWriter())
...
def test_verify_and_close_leaves_content_type_entries_open() -> None:
    ...
    book.update_error_book(
        [_issue(error_type="unsupported_facts", phenomenon="no source", refs=["claim-1"])],
        batch_id=1,
        writer=_FakeWriter(),
    )
...
def test_verify_and_close_closes_incomplete_page_once_complete() -> None:
    ...
    writer = _FakeWriter()
    book.update_error_book(
        [_issue(error_type="incomplete_pages", phenomenon="missing summary", refs=["water"])],
        batch_id=1,
        writer=writer,
    )
    writer.write_concept(Concept(slug="water", concept_title="Water", summary="A compound."))
...
def test_verify_and_close_closes_malformed_ref_once_well_formed() -> None:
    ...
    writer = _FakeWriter()
    book.update_error_book(
        [_issue(error_type="malformed_refs", phenomenon="bad ref", refs=["claim-1"])], batch_id=1, writer=writer
    )
    writer.write_claim(
